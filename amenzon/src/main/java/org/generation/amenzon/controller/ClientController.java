...
@RestController //Decimos que es una API Rest que trabaja con métodos GET, PUT, POST, DELETE
@RequestMapping(path = "/users/clients") //Aquí defino la ruta de nuestra API, es decir, el endpoint...
//http://localhost:8080/users/clients
@CrossOrigin(origins = "*", methods = {RequestMethod.GET, RequestMethod.POST,RequestMethod.DELETE,RequestMethod.PUT }) //Mediante origen cruzado puedo manejar problemas de seguridad. Me permite acceder desde cualquier origina (dominio) a los métodos HTTP
public class ClientController {
	...
	//Conecto mi Contoller con mi Service mediante Autowired (inyección de dependencias) y solo necesito traer lo que Service puede hacer
	...
	/*Lo comento porque voy a crear un nuevo GETbyId (linea55)
	//Conectar mi Service y Controller para buscar un cliente por Id.
	//Debo indicar el URI específico del id.
	//Debo indicar cuál será el PaathVariable y el tipo de dato
	@GetMapping(path = "{id}")
...
//--------------------------------------------------------------------------------------------------//
	...
	//Método para eliminar clientes por Id. Necesito definir la URI correspondiente al Id, el @PathVariable y el tipo de datos sobre el cual actúa
